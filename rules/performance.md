...
Memoize only when a child actually re-renders too often. Split so only the part that changes updates (`useState` at the leaf). Clean up timers, listeners, and overlay entries. `TpSnackbar` already guards double-remove. `OverlayHost` is the one process-wide overlay list — do not allocate a second portal.
...
Images: pass layout width/height to `TpImage` so decode size is bounded (`src/ui/theme/tpImageCache.ts`). Do not add an image package. Skip unusable URLs. SVGs through `TpSvgIcon`. Precache only when the next screen will show the same bytes immediately.

Network: one in-flight request per user intent unless the product needs parallel independent resources. Cancel superseded typeahead. Do not fetch in render or refresh on every re-render. Parse once at the boundary.
