...
Timeouts: `__tests__/core/http/http.test.ts`. Jest replaces `fetch` with `ScriptedHttp` ([testing.md](testing.md)). Cancel: Contact and login leave-while-sending. User retry: `__tests__/features/contact/useContact.test.ts`. Login offline / timeout / API down: `__tests__/features/login/submitLogin.test.ts` and `useLogin.test.ts`.
